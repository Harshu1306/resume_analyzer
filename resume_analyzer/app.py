...
def process_ats_diagnostics(file_obj, job_description: str):
    ...
    skills_output = ", ".join(profile["skills"]) if profile["skills"] else "None explicitly found."

    results_markdown = f"""
    <div style="background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.3); padding: 20px; border-radius: 8px;">
        <h2 style='margin: 0 0 15px 0; color: #818cf8;'>🎯 Overall ATS Score: {ats_score}/10</h2>
...
with gr.Blocks(theme=custom_theme, css=custom_css, title="Career Assistant") as demo:
    ...
    with gr.Tabs():
        with gr.TabItem("ATS System"):
            gr.Markdown("<p style='color: #94a3b8; margin-bottom: 20px;'></p>")
            ...
            with gr.Row(equal_height=True):
                ...
                with gr.Column(scale=1):
                    ...
                    skills_display    = gr.Textbox(label="Capabilities", interactive=False)
